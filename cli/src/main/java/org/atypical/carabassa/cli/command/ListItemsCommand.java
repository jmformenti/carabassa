...
/** Lists items within a dataset. */
@Component
@Command(name = "items", description = "list items.")
public class ListItemsCommand implements Callable<Integer> {

  private static final String OUTPUT_FORMAT = "%8s\t%5s\t%50s\t%6s\t%19s\t%19s\t%19s\t%32s%n";
      ...
      if (datasetId != null) {
        ...
        if (!items.isEmpty()) {
          ...
          for (ItemRepresentation itemRepresentation : items) {
            System.out.format(
                OUTPUT_FORMAT,
                itemRepresentation.getId(),
                itemRepresentation.getType(),
                itemRepresentation.getFilename(),
                itemRepresentation.getFormat(),
                ...
                itemRepresentation.getModification() == null
                    ? ""
                    : DateFormatter.toLocalDateFormatted(
                        itemRepresentation.getModificationAsZoned(ZoneId.systemDefault().getId())),
                itemRepresentation.getArchiveTime() == null
                    ? ""
                    : DateFormatter.toLocalDateFormatted(
                        itemRepresentation.getArchiveTimeAsZoned(ZoneId.systemDefault().getId())),
