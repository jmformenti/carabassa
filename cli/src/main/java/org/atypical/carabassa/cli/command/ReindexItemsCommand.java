...
/** Reindexes items for a dataset or search filter. */
@Component
...
public class ReindexItemsCommand implements Callable<Integer> {
    ...
    try {
      ...
      if (datasetId != null) {
        ...
        if (!items.isEmpty()) {
          ...
          cmdLogger.info(
              String.format("Reindexed %d of %d items (%d error)", reindex, total, error));
          cmdLogger.info("done.");
        } else {
  ...
  private void reindex(Long datasetId, ItemRepresentation itemRepresentation) {
    try {
      cmdLogger.info(
          String.format(
              "Reindex item %s ( %d / %d ) ...", itemRepresentation.getId(), ++count, total));
