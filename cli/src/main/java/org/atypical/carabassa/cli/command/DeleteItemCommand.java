...
/** Deletes items matching a dataset search query. */
@Component
...
public class DeleteItemCommand implements Callable<Integer> {
  ...
  @Option(
      names = {"-f", "--force"},
      description = "no ask before deleting.",
      defaultValue = "false")
  ...
  public Integer call() {
    try {
      ...
      if (datasetId != null) {
        ...
        if (!items.isEmpty()) {
          if (force
              || InteractiveCommand.doConfirm(
                  String.format(
                      "This action cannot be reversed. You are going to delete %d items in dataset"
                          + " '%s'. Are you sure? [y|N] ",
                      items.size(), dataset))) {
            total = items.size();
            ...
            cmdLogger.info(
                String.format("Deleted %d of %d items (%d error)", deleted, total, error));
          }
        } else {
  ...
  private void deleteItem(Long datasetId, ItemRepresentation itemRepresentation) {
    try {
      cmdLogger.info(
          String.format(
              "Delete item %s ( %d / %d ) ...", itemRepresentation.getId(), ++count, total));
