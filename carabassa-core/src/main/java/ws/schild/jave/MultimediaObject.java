...
public class MultimediaObject {
	...
	/**
	 * When true, we try to not read the source more than once One of the side
	 ...
	 *                    One of the side effects is, that no progressbar is
	 *                    available.
	 */
	 ...
	 * {@link ws.schild.jave.process.ffmpeg.FFMPEGProcess}.
	 *
	 * @param input   Input file for creating MultimediaObject
	 ...
	 */
	/*
	 * TODO: Refactor all parsing logic to a versioned parsing utility so we can
	 * detect FFMPEG version programmatically/support multiple runtime versions and
	 * consolidate parsing in one location.
	 */
	private MultimediaInfo parseMultimediaInfo(String source, RBufferedReader reader)
		...
		Pattern p22 = Pattern.compile("^\\s*Duration: (\\d\\d):(\\d\\d):(\\d\\d)\\.(\\d\\d).*$",
				Pattern.CASE_INSENSITIVE);
				...
				case 1: {
					...
					if (m1.matches()) {
						if (m2.matches()) {
							...
							long duration = (dec * 10L) + (seconds * 1000L) + (minutes * 60L * 1000L)
									+ (hours * 60L * 60L * 1000L);
